...
} from "../universal/RACEMintableERC20.sol";
...
contract StandardBridgeTester is StandardBridge {
    ...
    function isRACEMintableERC20(address _token) external view returns (bool) {
        return _isRACEMintableERC20(_token);
    }
 ...
 * @notice Simple implementation of the legacy RACEMintableERC20.
 */
contract LegacyMintable is ERC20, ILegacyMintableERC20 {
     ...
     *         it is how the actual legacy RACE mintable token does the
...
contract StandardBridge_Stateless_Test is CommonTest {
    ...
    RACEMintableERC20 internal mintable;
    ...
    function setUp() public override {
        ...
        mintable = new RACEMintableERC20({
            _bridge: address(0),
    ...
    /**
     * @notice Test coverage for identifying RACEMintableERC20 tokens.
     *         This function should return true for both modern and legacy
     *         RACEMintableERC20 tokens and false for any accounts that
     *         do not implement the interface.
     */
    function test_isRACEMintableERC20_succeeds() external {
        // Both the modern and legacy mintable tokens should return true
        assertTrue(bridge.isRACEMintableERC20(address(mintable)));
        assertTrue(bridge.isRACEMintableERC20(address(legacy)));
        // A regular ERC20 should return false
        assertFalse(bridge.isRACEMintableERC20(address(erc20)));
        // Non existent contracts should return false and not revert
        assertEq(address(0x20).code.length, 0);
        assertFalse(bridge.isRACEMintableERC20(address(0x20)));
    }
