...
contract RACEPortal_Test is Portal_Initializer {
    ...
    /**
     * @notice The RACEPortal can be paused by the GUARDIAN
     */
    ...
    /**
     * @notice The RACEPortal reverts when an account that is not the
    ...
    function test_pause_onlyGuardian_reverts() external {
        ...
        vm.expectRevert("RACEPortal: only guardian can pause");
        vm.prank(alice);
        op.pause();
    ...
    /**
     * @notice The RACEPortal can be unpaused by the GUARDIAN
     */
    ...
    /**
     * @notice The RACEPortal reverts when an account that is not
    ...
    function test_unpause_onlyGuardian_reverts() external {
        ...
        assertTrue(op.GUARDIAN() != alice);
        vm.expectRevert("RACEPortal: only guardian can unpause");
        vm.prank(alice);
        op.unpause();
    ...
    function test_depositTransaction_contractCreation_reverts() external {
        ...
        vm.expectRevert("RACEPortal: must send to address(0) when creating a contract");
    ...
    function test_depositTransaction_largeData_reverts() external {
        ...
        vm.expectRevert("RACEPortal: data too large");
    ...
    function test_depositTransaction_smallGasLimit_reverts() external {
        vm.expectRevert("RACEPortal: gas limit too small");
        op.depositTransaction({
            _to: address(1),
            _value: 0,
            _gasLimit: 0,
...
contract RACEPortal_FinalizeWithdrawal_Test is Portal_Initializer {
    ...
    function callPortalAndExpectRevert() external payable {
        vm.expectRevert("RACEPortal: can only trigger one withdrawal per transaction");
    ...
    function test_proveWithdrawalTransaction_paused_reverts() external {
        ...
        vm.expectRevert("RACEPortal: paused");
    ...
    // Test: proveWithdrawalTransaction cannot prove a withdrawal with itself (the RACEPortal) as the target.
    function test_proveWithdrawalTransaction_onSelfCall_reverts() external {
        _defaultTx.target = address(op);
        vm.expectRevert("RACEPortal: you cannot send messages to the portal contract");
        op.proveWithdrawalTransaction(
            _defaultTx,
    ...
    function test_proveWithdrawalTransaction_onInvalidOutputRootProof_reverts() external {
        ...
        vm.expectRevert("RACEPortal: invalid output root proof");
        op.proveWithdrawalTransaction(
            _defaultTx,
            _proposedOutputIndex,
            _outputRootProof,
    ...
    function test_proveWithdrawalTransaction_replayProve_reverts() external {
        ...
        vm.expectRevert("RACEPortal: withdrawal hash has already been proven");
        op.proveWithdrawalTransaction(
            _defaultTx,
            _proposedOutputIndex,
            _outputRootProof,
    ...
    function test_proveWithdrawalTransaction_replayProveChangedOutputRoot_succeeds() external {
        ...
        assembly {
            ...
            mstore(0x20, 52) // 52 is the slot of the `provenWithdrawals` mapping in the RACEPortal
        ...
        external
    {
        ...
        assembly {
            mstore(0x00, sload(_withdrawalHash.slot))
            mstore(0x20, 52) // 52 is the slot of the `provenWithdrawals` mapping in RACEPortal
            slot := keccak256(0x00, 0x40)
        }
    ...
    function test_finalizeWithdrawalTransaction_ifWithdrawalNotProven_reverts() external {
        ...
        vm.expectRevert("RACEPortal: withdrawal has not been proven yet");
        op.finalizeWithdrawalTransaction(_defaultTx);
    ...
    function test_finalizeWithdrawalTransaction_ifWithdrawalProofNotOldEnough_reverts() external {
        ...
        vm.expectRevert("RACEPortal: proven withdrawal finalization period has not elapsed");
        op.finalizeWithdrawalTransaction(_defaultTx);
    ...
    function test_finalizeWithdrawalTransaction_timestampLessThanL2OracleStart_reverts() external {
        ...
        vm.expectRevert(
            "RACEPortal: withdrawal timestamp less than L2 Oracle starting timestamp"
        );
        op.finalizeWithdrawalTransaction(_defaultTx);
    ...
    function test_finalizeWithdrawalTransaction_ifOutputRootChanges_reverts() external {
        ...
        // Attempt to finalize the withdrawal
        vm.expectRevert(
            "RACEPortal: output root proven is not the same as current output root"
        );
        op.finalizeWithdrawalTransaction(_defaultTx);
    ...
    function test_finalizeWithdrawalTransaction_ifOutputTimestampIsNotFinalized_reverts() external {
        ...
        // Attempt to finalize the withdrawal
        vm.expectRevert("RACEPortal: output proposal finalization period has not elapsed");
        op.finalizeWithdrawalTransaction(_defaultTx);
    ...
    function test_finalizeWithdrawalTransaction_onReplay_reverts() external {
        ...
        vm.expectRevert("RACEPortal: withdrawal has already been finalized");
        op.finalizeWithdrawalTransaction(_defaultTx);
    }
    ...
    function testDiff_finalizeWithdrawalTransaction_succeeds(
        ...
        vm.assume(
            _target != address(op) && // Cannot call the RACE portal or a contract
...
contract RACEPortalUpgradeable_Test is Portal_Initializer {
    ...
    function test_params_initValuesOnProxy_succeeds() external {
        RACEPortal p = RACEPortal(payable(address(proxy)));
    ...
    function test_initialize_cannotInitProxy_reverts() external {
        ...
        RACEPortal(payable(proxy)).initialize(false);
    }

    function test_initialize_cannotInitImpl_reverts() external {
        vm.expectRevert("Initializable: contract is already initialized");
        RACEPortal(opImpl).initialize(false);
    }
